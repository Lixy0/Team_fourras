...
// We'll create saves in our sprite group for each object in our map
  ...
  this.physics.add.overlap(this.player, this.saves, sauvegarde,null, this)
...
// We'll create moves in our sprite group for each object in our map
  ...
  map.getObjectLayer('Mouvable').objects.forEach((move) => {
    ...
    moveSprite.name=nom++
    trucsBleu.push(moveSprite);
  });
  this.physics.add.collider(this.player, this.moves, pousser,null, this)
  this.physics.add.collider(this.moves,this.moves)
  this.cameras.main.startFollow(this.player);
}
function  sauvegarde (player, saves){
  console.log("current",this.currentSaveX,this.currentSaveY)
  this.currentSaveX = player.x
  this.currentSaveY = player.y
  saves.body.enable=false;
}
function pousser(player,moves){
  if (player.body.touching.right && moves.body.touching.left){
    moves.x+=1
  }
  if (player.body.touching.left && moves.body.touching.right){
    moves.x-=1
  }
